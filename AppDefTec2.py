...
# Definir función para cargar y preprocesar las imágenes
def cargar_imagenes():
    # Cargar imágenes
    # en este caso no partimos los datos ya que validación y entrenamiento se encuentran ya separados
    image_size = (180, 180)
    batch_size = 128
    data_train = st.file_uploader('Selecciona carpeta de entrenamiento')
    ...
    numclasses=data.train.num_classes
    data_test = st.file_uploader('Selecciona carpeta de validacion')
...
def crear_modelo(filtros, tamaño, pool_size, neuronas):
    # Crear modelo
    model = Sequential([
...
def train_page:
  ...
  # Cargar imágenes
  train_generator, validation_generator = cargar_imagenes()
  ...
  model, history = crear_modelo(num_filters, filter_size, pool_size, num_neurons)
  ...
  ax[1].plot(history.history['val_loss'], label='Pérdida en
